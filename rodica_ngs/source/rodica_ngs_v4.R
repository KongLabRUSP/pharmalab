...
# |---------------------------------------------------------------------------------|
# sink(file = "tmp/log_rodica_ngs_v4.txt")
...
# names(rna)[1] <- "gene"
# Raw expressions----
rna.all <- fread("rodica_ngs/data/featurecounts.results.human_for_prof.Yen.csv")
rna.all <- rna.all[, c("Geneid",
                       "C.dedup.bam",
                       "RA.dedup.bam",
                       "SFN.dedup.bam",
                       "SRA.dedup.bam",
                       "UA.dedup.bam",
                       "URA.dedup.bam")]
names(rna.all)[names(rna.all) == "Geneid"] <- "gene"
names(rna.all)[2:ncol(rna.all)] <- gsub(pattern = ".dedup.bam",
                                        replacement = "",
                                        x = names(rna.all)[2:ncol(rna.all)])
rna.all

# Normalize data----
mat <- data.frame(sample = colnames(rna.all)[-1],
                  trt = colnames(rna.all)[-1],
                  repl = factor(rep(1, ncol(rna.all) - 1)))
mat

dtm <- as.matrix(rna.all[, -1, with = FALSE])
rownames(dtm) <- rna.all$gene
head(dtm)
...
rna <- data.table(gene = rna.all$gene,
                  counts(dds, 
                         normalized=TRUE))
rna

# Selected genes----
rna.select <- fread("rodica_ngs/data/Heatmap Output (3).csv")[,  1:6]
names(rna.select)[1] <- "gene"

# Keep only the genes selected in the prior step (by Petruta)----
rna <- rna[gene %in% unique(rna.select$gene), ]
rna

# Log-transform, center and scale----
rna[, 2:ncol(rna)] <- lapply(rna[, 2:ncol(rna)],
                             function(a) {
                               a <- log(a + 1)
                             })
rna
...
p1 <- ggplot(data = dtp2) +
  ...
  geom_tile(aes(x =  as.numeric(Treatment),
                ...
                fill = RNA),
            color = "white") +
  geom_text(data = dtp2[Treatment == "C", ],
            aes(x = rep(5.75,
                        nlevels(gene)),
                y = gene,
                angle = 90 + seq(from = 0,
  ...
  scale_y_discrete("",
                   expand = c(0, 0)) +
  ggtitle("RNA Normalized Counts in Differentially Expressed Genes") + 
...
rna.fpm <- data.table(gene = rna.all$gene,
                      fpm(dds,
                          robust = FALSE))
rna.fpm

# Keep only the genes selected in the prior step (by Petruta)----
rna.fpm <- rna.fpm[gene %in% unique(rna.select$gene), ]
rna.fpm

# Log-transform, center and scale----
rna.fpm[, 2:ncol(rna.fpm)] <- lapply(rna.fpm[, 2:ncol(rna.fpm)],
                                     function(a) {
                                       a <- log(a + 1)
                                     })
rna.fpm
...
# Heatmap----
# Distances----
tmp <- as.matrix(rna.fpm[, -1])
rownames(tmp) <- rna.fpm$gene
geneDist <- dist(tmp)
...
dtp1 <- segment(ddata)
# Hitmap data----
dtp2 <- melt.data.table(rna.fpm,
                        id.vars = 1,
                        measure.vars = 2:ncol(rna.fpm),
                        variable.name = "Treatment",
                        value.name = "RNA")
dtp2$gene <- factor(dtp2$gene,
                    levels = ddata$labels$label)
...
p1 <- ggplot(data = dtp2) +
  ...
  scale_y_discrete("",
                   expand = c(0, 0)) +
  ggtitle("RNA FPM in Differentially Expressed Genes") + 
  theme(plot.title = element_text(hjust = 0.5),
        axis.title.x = element_blank(),
        axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
...
tiff(filename = "tmp/heatmap.fpm.tiff",
     height = 10,
     width = 8,
     units = 'in',
     res = 300,
