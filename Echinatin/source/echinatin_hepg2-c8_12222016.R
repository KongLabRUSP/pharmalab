...
# Data----
# 1. BCA----
bca <- fread("Echinatin/data/HepG2C8_SFN_Ech_BCA_12222016.csv",
             sep = ",",
             header = TRUE)

# BCA Maps----
bca.map <- fread("Echinatin/data/Map_HepG2C8_SFN_Ech_BCA_12072016.csv",
                 sep = ",",
                 header = TRUE)

# 2. Luciferase----
luc <- fread("Echinatin/data/HepG2C8_SFN_Ech_Luciferace_12222016.csv",
             sep = ",",
             header = TRUE)
...
dt1.1 <- subset(dt1, Treatment != "BCA_Only")
...
# ATTENTION! REMOVE OUTLIERS IN 12/07/2016 EXPERIMENT
alb$BCA[17] <- NA
alb$BCA[22] <- NA
...
dt2$Treatment <- factor(dt2$Treatment, levels = c("Echinatin_80uM",
                                                  "Echinatin_40uM",
                                                  "Echinatin_20uM",
                                                  "SFN_10uM",
                                                  "SFN_5uM",
...
ggplot(dt2,
       aes(x = Treatment,
           y = Pct_DMSO,
           fill = Treatment)) +
  geom_errorbar(aes(ymin = Pct_DMSO, 
                    ymax = Pct_DMSO + SD),
                colour = "black", 
                width = .1) +
  geom_bar(position = position_dodge(),
