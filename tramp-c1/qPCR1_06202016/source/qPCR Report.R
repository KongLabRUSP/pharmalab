setwd("C:\\svn_mirror\\PhD\\Lab\\qPCR1_06202016")
require(data.table)
require(ggplot2)
# require(reshape2)
# require(knitr)
...
d1 <- dd$`2016-06-30-davit.csv`
d1 <- dd$`2016-07-05-davit.csv`
names(d1) <- c("Sample", "Target", "Mean", "SD")
d1 <- subset(d1, Sample != "h2o")
d1$Sample <- factor(d1$Sample, levels = unique(d1$Sample))
d1$Target <- factor(d1$Target, levels = unique(d1$Target))
d1
...
plates <- data.table(Hour = rep(c(24, 48, 72), each = 8),
                     Plate = rep(rep(1, each = 8), 3),
                     Reading = rep(rep(1, each = 8), 3),
                     Repl = rep(c(0, rep(1:3, each = 2), 0), 3),
                     plates)
names(plates)[5:ncol(plates)] <- c("Row", paste("C", 1:12, sep = ""))
plates

maps <- fread("MTS4_06062016_06082016\\data\\TC1_MTS_Exp4_Map_06062016.csv",
              sep = ",", 
              header = TRUE)
...
#ANALYSIS I: Genistein
dd.gen <- subset(saved.dd, select = c(1:3, 5, 7, 9))
dd.gen$dilut <- 90000/2^as.numeric(as.character(dd.gen$Concentration))
